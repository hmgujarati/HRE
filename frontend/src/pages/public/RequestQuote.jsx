      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="border border-zinc-200 bg-white">
            ...
            <div className="border-t-2 border-zinc-300 px-5 py-3 flex items-center justify-end gap-8 text-sm">
              <div><span className="text-zinc-500 mr-2">Subtotal:</span><span className="font-mono">₹{totals.subtotal.toFixed(2)}</span></div>
              <div><span className="text-zinc-500 mr-2">GST:</span><span className="font-mono">₹{totals.gst.toFixed(2)}</span></div>
              <div className="font-heading font-black text-xl">₹{totals.grand.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
            </div>
          </div>
          <div className="flex justify-end gap-2">
            ...
            <button onClick={finalise} disabled={busy} className="bg-[#FBAE17] hover:bg-[#E59D12] text-black font-bold uppercase tracking-wider text-xs px-6 py-3 flex items-center gap-2 disabled:opacity-60" data-testid="public-finalise-btn">
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s} className="flex-1 flex items-center gap-3">
            <div className={`w-7 h-7 flex items-center justify-center text-xs font-bold ${active ? 'bg-[#FBAE17] text-black' : 'bg-zinc-100 text-zinc-400'}`}>{idx}</div>
            <div className={`text-[10px] uppercase tracking-wider font-bold ${active ? 'text-[#1A1A1A]' : 'text-zinc-400'}`}>{s}</div>
